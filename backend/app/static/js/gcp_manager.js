    ...
    try {
        ...
        projects.forEach(p => {
            ...
            projectRow.innerHTML = `
                <div style="display: flex; flex-direction: column; gap: 2px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; max-width: 80%;">
                    <span style="font-weight: 600; font-size: 0.88rem; color: var(--text-primary);">📁 ${p.project_id}</span>
...
// Upload a GCP service account JSON key file
async function uploadGcpProjectKey(file) {
    ...
    try {
        ...
        if (response.ok) {
            showToast(`Added GCP Project '${data.project_id}' successfully!`);
            loadGcpProjects();
        } else {
    ...
    // Clear input
    document.getElementById('gcp-file-input').value = '';
}
